          ...
          return (
            <div key={col.status} className="flex flex-col">
              ...
              <div className={cn("scrollbar-thin flex min-h-[120px] flex-1 flex-col gap-2 rounded-[var(--radius-lg)] border-t-2 bg-bg-subtle/40 p-2", col.accent)}>
                {items.map((t) => (
                  <TaskCard key={t.id} task={t} onMove={moveTask} showClient={isAllClients} />
                ))}
                {items.length === 0 && (
...
function TaskCard({ task, onMove, showClient }: { task: CrmTask; onMove: (id: string, status: TaskStatus) => void; showClient: boolean }) {
  ...
  return (
    <Card className="cursor-pointer p-3 transition-shadow hover:shadow-[var(--shadow-md)]">
